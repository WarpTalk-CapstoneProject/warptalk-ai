...
class FakeSource:
    ...
    def __init__(self, fail_at_frame: int | None = None):
...
@pytest.mark.asyncio
async def test_a_clean_run_reports_the_whole_buffer():
    audio = pcm(5)
    source = FakeSource()
    sent = await publisher()._capture_from(source, audio, SAMPLE_RATE)
    assert sent == len(audio)


@pytest.mark.asyncio
async def test_a_break_reports_where_it_stopped_not_failure():
...
@pytest.mark.asyncio
async def test_resuming_speaks_the_line_once_and_whole():
    audio = pcm(10)

    first = FakeSource(fail_at_frame=4)
    sent = await publisher()._capture_from(first, audio, SAMPLE_RATE)

    second = FakeSource()
    sent += await publisher()._capture_from(second, audio[sent:], SAMPLE_RATE)
